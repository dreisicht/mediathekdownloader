...
OUTPUT_DIR = Path.home() / "Downloads/Suits"
...
def get_html(season: int) -> str:
  season_num = str(season).zfill(2)
  content = set()

  for page_num in range(1, 5):
    url = f"https://mediathekviewweb.de/#query=suits%2C%20%22(s{season_num}%2FE%22&sortOrder=asc&page={page_num}"
...
def main() -> None:
  for season in range(1, 2):
    urls = extract_urls_from_html(get_html(season))
